...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    params.debug = debug_
    # mmtrack = create_tracker(script_name, params, 'lasot')  # "GTOT" # dataset_name
    mmtrack = create_tracker(script_name, params, 'visevent')  # "GTOT" # dataset_name
    tracker = VEGETA_RGBE(tracker=mmtrack)
    ...
    for frame_idx, (rgb_path, E_path) in enumerate(zip(RGB_img_list, E_img_list)):
        ...
        if frame_idx == 0:
            ...
            # image = get_x_frame(rgb_path, E_path, dtype='color')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml_name', type=str, default='uetrack_base', help='Name of tracking method.')
    ...
    if dataset_name == 'VisEvent':
        seq_home = '/media/kb/2T5/trackingdata/trackingdata_m/VisEvent/test'
