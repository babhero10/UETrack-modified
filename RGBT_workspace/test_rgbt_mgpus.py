...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    tracker = VEGETA_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='uetrack', help='Name of tracking method(ostrack, prompt, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='uetrack_base', help='Name of tracking method.')  # vitb_256_mae_ce_32x4_ep300 vitb_256_mae_ce_32x4_ep60_prompt_i32v21_onlylasher_rgbt
    ...
    if dataset_name == 'GTOT':
        seq_home = '/home/cx/cx2/mmtdata/GTOT/Multi_Modal_RGBT_dataset_CSR'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/media/kb/2T5/trackingdata/trackingdata_m/RGB-T234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/media/kb/2T5/trackingdata/trackingdata_m/LasHeR/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/home/cx/cx2/mmtdata/VTUAV/test/short-term'
        with open(join(join(seq_home, 'VTUAV-ST.txt')), 'r') as f:
            seq_list = f.read().splitlines()
    elif dataset_name == 'VTUAVLT':
        seq_home = '/home/cx/cx2/mmtdata/VTUAV/test/long-term'
