...
class BatchedSwiGLUExpert(nn.Module):
    def __init__(self, num_experts, in_features, hidden_features=None, out_features=None,
        ...
        # Stack weights and biases for all experts in a single parameter
        self.w1_weight = nn.Parameter(torch.stack([
            nn.init.xavier_uniform_(torch.empty(hidden_features, in_features))
            for _ in range(num_experts)
        ]))  # (e, hidden, in)
        self.w1_bias   = nn.Parameter(torch.zeros(num_experts, hidden_features))
        self.w2_weight = nn.Parameter(torch.stack([
            nn.init.xavier_uniform_(torch.empty(hidden_features, in_features))
            for _ in range(num_experts)
        ]))  # (e, hidden, in)
        self.w2_bias   = nn.Parameter(torch.zeros(num_experts, hidden_features))

        self.act = act_layer()
        self.ffn_ln = norm_layer(hidden_features) if subln else nn.Identity()

        self.w3_weight = nn.Parameter(torch.stack([
            nn.init.xavier_uniform_(torch.empty(out_features, hidden_features))
            for _ in range(num_experts)
        ]))  # (e, out, hidden)
        self.w3_bias   = nn.Parameter(torch.zeros(num_experts, out_features))
    ...
    def forward(self, x):
        ...
        e, b, s, _ = x.shape
        # Linear 1: x1 = w1 x + b1
        x1 = torch.einsum('ebsi, ehi -> ebsh', x, self.w1_weight) + self.w1_bias.view(e, 1, 1, -1)
        # Linear 2
        x2 = torch.einsum('ebsi, ehi -> ebsh', x, self.w2_weight) + self.w2_bias.view(e, 1, 1, -1)
        # SwiGLU activation
        hidden = self.act(x1) * x2
        # Optional sub-layer norm
        hidden = self.ffn_ln(hidden)
        # Final linear
        out = torch.einsum('ebsh, eoh -> ebso', hidden, self.w3_weight) + self.w3_bias.view(e, 1, 1, -1)
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        return self.l2norm(x) * self.scale * self.gamma
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if swiglu:
            ...
            if layer_index in moe_layer:
                self.experts = BatchedSwiGLUExpert(
    ...
    def forward(self, x, task_index, rel_pos_bias=None, attn_mask=None):
        ...
        if self.gamma_2 is None:
            ...
            if self.postnorm:
                ...
                if self.layer_index in self.moe_layer:
                    B, T, D = x.shape
                    E = self.num_experts
                    x_norm = self.norm2(x)
                    pad_len = (-T) % E
                    if pad_len:
                        x_pad = F.pad(x_norm, (0, 0, 0, pad_len))  # (B, T+pad_len, D)
                        valid_mask = torch.arange(T + pad_len, device=x.device).unsqueeze(0) < T
                    else:
                        x_pad = x_norm
                        valid_mask = None
                    TP = x_pad.size(1)
                    ...
                    # local aggregation
                    x_reshaped = x_pad.view(B, S, E, D)  # (B, S, E, D)
                    if valid_mask is not None:
                        mask_seg = valid_mask.view(1, TP, 1).view(1, S, E, 1)  # (1, S, E, 1)
                        x_reshaped = x_reshaped * mask_seg
                        counts = mask_seg.sum(dim=2).clamp(min=1)  # (1, S, 1)
                        x_mean = x_reshaped.sum(dim=2) / counts  # (B, S, D)
                    else:
                        x_mean = x_reshaped.mean(dim=2)  # (B, S, D)

                    # expert embeddings
                    # yields (B, E, S, D)
                    expert_tokens = self.to_slot_embeds(x_mean)
                    ES = E * S

                    #Compute logits
                    expert_tokens_flat = expert_tokens.reshape(B, ES, D)  # (B, ES, D)
                    logits = x_pad.matmul(expert_tokens_flat.transpose(1, 2))  # (B, TP, ES)
                    logits = logits.view(B, TP, E, S)

                    #Mask padded positions
                    if valid_mask is not None:
                        mask4 = valid_mask.view(1, TP, 1, 1)
                        logits = logits.masked_fill(~mask4, -torch.finfo(logits.dtype).max)

                    #Softmax routing
                    dispatch = F.softmax(logits, dim=1)  # (B, TP, E, S)

                    combine = F.softmax(logits.view(B,TP,ES), dim=-1)  # (B, TP, ES)

                    # Dispatch to experts
                    disp_flat = dispatch.view(B, TP, ES).transpose(1, 2)  # (B, ES, TP)
                    experts_in_flat = disp_flat.matmul(x_pad)  # (B, ES, D)
                    experts_in = experts_in_flat.view(B, E, S, D).permute(1, 0, 2, 3)

                    #Expert forward
                    experts_out = self.experts(experts_in)  # (E, B, S, D)

                    #Combine expert outputs
                    out_flat = experts_out.permute(1, 0, 2, 3).reshape(B, ES, D)
                    x_out = combine.matmul(out_flat)  # (B, TP, D)
                    x_out = x_out[:, :T, :]
                    x = x + self.drop_path(self.gamma_2 * x_out)
                else:
...
class Fast_iTPN(nn.Module):
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        if self.cls_token is not None:
            ...
            xz = torch.cat([cls_tokens, xz], dim=1)#cls,x,z,text

        return xz

    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        xz = self.prepare_tokens_with_masks(template_list, search_list, template_anno_list, text_src, task_index)##cls,x,z,text
        xz = self.pos_drop(xz)
        feature_list = []
        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None
        for i,blk in enumerate(self.blocks[-self.num_main_blocks:]):
            xz = checkpoint.checkpoint(blk, xz, task_index, rel_pos_bias) if self.grad_ckpt else blk(xz, task_index, rel_pos_bias)
            if i + 1 in self.distill_layer:
                feature_list.append(xz)

        xz = self.norm(xz)

        if self.fc_norm is not None:
            return self.fc_norm(xz),feature_list
        else:
            return xz,feature_list

    def forward(self, template_list, search_list, template_anno_list, text_src, task_index):
        xz,feature_list = self.forward_features(template_list, search_list, template_anno_list, text_src, task_index)
        out = [xz]
    ...
    for key in model.state_dict().keys():
        if 'experts' in key:
            if 'w1' in key or 'w2' in key or 'w3' in key:
                key_r = key.replace('experts','mlp').replace('_','.')
                if 'weight' in key:
                    n_expert,_,_ = model.state_dict()[key].shape
                    value_r = state_dict[key_r].repeat(n_expert,1,1)
                    state_dict[key] = value_r
                elif 'bias' in key:
                    n_expert,_= model.state_dict()[key].shape
                    value_r = state_dict[key_r].repeat(n_expert,1)
                    state_dict[key] = value_r
                else:
                    raise KeyError
            else:
                key_r=key.replace('experts','mlp')
                state_dict[key] = state_dict[key_r]
    model.load_state_dict(state_dict, strict=False)

@register_model
def fastitpnt_layer2(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy",
                     num_experts=1,moe_layer=[3],distill_layer=[],**kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=6, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=2, num_heads=6, bridge_mlp_ratio=3.,
    ...
    return model
@register_model
def fastitpnt_layer4(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy",
                     num_experts=1,moe_layer=[3],distill_layer=[],**kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=6, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=4, num_heads=6, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
...
@register_model
def fastitpnt_layer6(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy",
                     num_experts=1,moe_layer=[3],distill_layer=[],**kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=6, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=6, num_heads=6, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
