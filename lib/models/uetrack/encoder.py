"""
Encoder modules: we use Fast-ITPN for the encoder.
"""

from torch import nn
from lib.utils.misc import is_main_process
from lib.models.uetrack import fastitpn as fastitpn_module
from lib.models.uetrack import fastitpn_teacher as fastitpn_teacher_module
...
class Encoder_Teacher(EncoderBase):
    """ViT encoder."""
    def __init__(self, name: str,
                 train_encoder: bool,
                 search_size: int,
                 ...
                 cfg=None):
        if "fastitpn" in name.lower():
            encoder = getattr(fastitpn_teacher_module, name)(
                pretrained=is_main_process(),
                search_size=search_size,
                template_size=template_size,
                drop_rate=0.0,
                ...
                distill_layer= cfg.TRAIN.DISTILL_LAYER_T,
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
...
def build_encoder_teacher(cfg):
    train_encoder = False
    encoder = Encoder_Teacher(cfg.TRAIN.TEACHER_TYPE, train_encoder,
                      cfg.DATA.SEARCH.SIZE,
                      cfg.DATA.TEMPLATE.SIZE,
                      [],cfg)
    return encoder
